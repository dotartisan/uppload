...
test("get a translation with multiple variables", () => {
    ...
    setI18N(lang);
    expect(translate("sayHello", ["Anand", "Chowdhary"])).toBe("Hello, Anand Chowdhary!");
});
...
[en, nl, hi, es, fr, IT, ru, tr, de, zhTW].forEach((lang, i) => {
    describe(`translation - lang ${i}`, () => {
        const language = flattenObject(lang);
        translationKeys.forEach((key) => {
            it(`has translation for ${key}`, () => expect(language[key]).toBeDefined());
        });
    });
});
