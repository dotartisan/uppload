...
import { Camera, Local, GIPHY, Pexels, Pixabay, Unsplash, NineGag, ArtStation, DeviantArt, Facebook, Flickr, Instagram, Pinterest, Screenshot, Twitter, URL, } from "../src";
...
describe("service template", () => {
    ...
    it("gives a string", () => expect(typeof service.template(serviceTemplateParams)).toBe("string"));
});
const uppload = new Uppload();
uppload.use(services);
for (const currentService of services) {
    describe(`${currentService.name} service`, () => {
        it("template is a function", () => expect(typeof currentService.template).toBe("function"));
        it("template gives a string", () => expect(typeof currentService.template(serviceTemplateParams)).toBe("string"));
        it("service is registered", () => expect(
        ...
        currentService.name === "camera"
            ...
            : uppload.services.includes(currentService)).toBeTruthy());
    });
}
