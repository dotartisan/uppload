...
test("adds an Uppload service", () => {
    ...
    expect(uppload.services.filter((services) => services.name === "instagram").length).toBe(1);
});
test("adds an Uppload effect", () => {
    uppload.use(new Crop());
    expect(uppload.effects.filter((effects) => effects.name === "crop").length).toBe(1);
});
describe("event emitter", () => {
    it("on should be a function", () => expect(typeof uppload.on).toBe("function"));
    it("off should be a function", () => expect(typeof uppload.off).toBe("function"));
...
//# sourceMappingURL=uppload.test.js.map